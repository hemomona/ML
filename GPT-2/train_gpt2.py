...
A thing I discovered:
    I used ~3B fineweb-edu data to train, and I used 54495 (~9.5 * 1 epoch and so ~28B data) by accident, leading to a
periodic loss in log/GPT2-124Mparams-3Bdata-lossandeval.png. Virtually this structure(B=16, T=512, ~28B data) resembles
Andrej's nanoGPT (B=64, T=1024, 10B data), but the result lags his a little, which might emphasize the importance of
input training dataset!

1. ways to improve speed:
    ...
    6. DistributedDataParallel, use multi GPU
2. ways to improve performance:
    ...
    5. bigger and better dataset, RedPajama, FineWebEDU

a comprehensive pipeline:
    training environment settings: ddp, cpu, single-GPU
    hyperparameter settings: total_batch_size, batch_size (B), sequence_length (T), max and min lr, warmup and max steps
    DataLoaderLite() init, (train_loader & val_loader): shards, inputs + 1 place = targets
    GPT() init:
        dataclass GPTConfig: block_size, vocab_size, n_layer, n_head, n_embd
        transformer:
            Embedding wte: vocab_size * n_embd
            Embedding wpe: block_size * n_embd
            Block h: n_layer
                LayerNorm ln_1: n_embd
                CauSalSelfAttention attn: NANOGPT_SCALE_INIT, c_attn Linear(n_embd, 3*embd), c_proj Linear(n_embd, n_embd)
                LayerNorm ln_2: n_embd
                MLP mlp: NANOGPT_SCALE_INIT, c_fc (n_embd, 4*n_embd), gelu, c_proj (4*n_embd, n_embd)
            LayerNorm ln_f: n_embd
        Linear lm_head: n_embd * vocab_size, final classifier
        wte = lm_head, weight sharing scheme
        _init_weights()
    GPT.configure_optimizers(): AdamW()
        once in a while:
            model.eval()
            evaluate using hellaswag
            generate answers
        model.train()
"""
import inspect
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # - a scheme includes more flops but is faster because of the avoidance of many flops related to HBM.
...
# otherwise, RuntimeError: User specified autocast device_type must be 'cuda' or 'cpu'
...
model.to(device)
# model = torch.compile(model)  # can be faster, however, it is not available before PyTorch 2.0
...
raw_model = model.module if ddp else model  # after wrap in DDP, there is no configure_optimizer in model
...
max_lr = 6e-4
min_lr = max_lr * 0.1
warmup_steps = 2042  # ratio compared between Andrej and me
max_steps = 54495  # total token / total_batch_size = 5722 I give the wrong max_steps, leading a periodic train loss

# create the log directory we will write checkpoints to and log to
log_dir = "log"
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, f"log.txt")
answer_file = os.path.join(log_dir, f"answer.txt")
...
for step in range(max_steps):
    ...
    if (step > 0 and step % 250 == 0) or last_step:
        ...
        tokens = torch.tensor(tokens, dtype=torch.long)  # (4,)
        tokens = tokens.unsqueeze(0).repeat(num_return_sequences, 1)  # (4, 8)
        xgen = tokens.to(device)
        # generate answers, now (B, T) == (4, 8)
        ...
        while xgen.size(1) < max_length:
            ...
            with torch.no_grad():  # no need to backward or cache tensors
                ...
                topk_probs, topk_indices = torch.topk(probs, 50, dim=-1)  # do top 50 sampling, DIM = -1
                ix = torch.multinomial(topk_probs, 1, generator=sample_rng)  # select a token, (5, 1)
    ...
    for micro_step in range(grad_accum_steps):
        ...
        x, y = x.to(device), y.to(device)
        with torch.autocast(device_type=device, dtype=torch.bfloat16):  # cause OOM in my Ampere GPU
            logits, loss = model(x, y)
    ...
    if master_process:
        # print(f"step{step:6d}, loss_accum: {loss_accum.item():6f}, norm: {norm:4f}, lr: {lr:6f}, "
        #       f"dt: {dt:.2f}s, tok/sec: {tokens_per_sec:.2f}")
        with open(log_file, "a") as f:
            f.write(f"{step: 6d} train {loss_accum.item():.6f}\n")
